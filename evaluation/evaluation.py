...
def setup(pred_result, data_dict, save_path, all_pred_id=np.arange(70), 
    ...
    for i, preds in enumerate(pred_result):
        ...
        if obj_co_occur is not None:
            ...
            '''
            ind = np.where(np.all(valid_preds <= 0.1, axis=1))
            sub_score[ind] = 0.
            obj_score[ind] = 0.
            valid_preds[ind] = 0.
            '''
            #print(valid_preds[1])
            #valid_preds = valid_preds + 0.8*(sub_score + obj_score)
            #valid_preds = valid_preds + (sub_score + obj_score)
            valid_preds = valid_preds + 1.0 *(sub_score + obj_score)
            #valid_preds = valid_preds * sub_score * obj_score
            #valid_preds = valid_preds * (sub_score + obj_score)
            # For lu
            #valid_preds = valid_preds + 0.9 * (sub_score + obj_score)
...
def top_recall_predicate(nre, all_tuple_path):
    with open(all_tuple_path, 'rb') as f:
        all_tuple = pickle.load(f)
    relation_triplet = all_tuple[0]
    relation_score = all_tuple[1]
